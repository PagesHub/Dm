package com.yang.sdk.threadpool;
...
import com.yang.sdk.threadpool.config.ThreadConfigs;
import com.yang.sdk.threadpool.deliver.AndroidDeliver;
import com.yang.sdk.threadpool.deliver.JavaDeliver;
import com.yang.sdk.threadpool.factory.SdkThreadFactory;
import com.yang.sdk.threadpool.utils.DelayTaskExecutor;
import com.yang.sdk.threadpool.utils.ThreadToolUtils;
import com.yang.sdk.threadpool.wrapper.CallableWrapper;
import com.yang.sdk.threadpool.wrapper.RunnableWrapper;
    ...
    private ExecutorService createPool(int type, int size, int priority) {
        switch (type) {
            case ThreadBuilder.TYPE_CACHE:
                ...
                return Executors.newCachedThreadPool(new SdkThreadFactory(priority));
            case ThreadBuilder.TYPE_FIXED:
                //线程数量固定的线程池，全部为核心线程，响应较快，不用担心线程会被回收。
                return Executors.newFixedThreadPool(size, new SdkThreadFactory(priority));
            case ThreadBuilder.TYPE_SCHEDULED:
                //有数量固定的核心线程，且有数量无限多的非核心线程，适合用于执行定时任务和固定周期的重复任务
                return Executors.newScheduledThreadPool(size, new SdkThreadFactory(priority));
            case ThreadBuilder.TYPE_SINGLE:
            default:
                //内部只有一个核心线程，所有任务进来都要排队按顺序执行
                return Executors.newSingleThreadExecutor(new SdkThreadFactory(priority));
        }
    }
