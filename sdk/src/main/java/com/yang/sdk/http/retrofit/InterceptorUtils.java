...
public class InterceptorUtils {
    ...
    public static Interceptor getCacheInterceptor() {
        return chain -> {
            ...
            if (!NetworkUtils.isNetworkConnected(SdkUtils.getApp())) {
        ...
        return chain -> {
            ...
            if (!NetworkUtils.isNetworkConnected(SdkUtils.getApp())) {
                request = request.newBuilder()
                        .cacheControl(CacheControl.FORCE_CACHE)
                        .build();
                LogUtils.dLog("addNetWorkInterceptor"+ "没有网络链接");
            }
            Response response = chain.proceed(request);
            if (NetworkUtils.isNetworkConnected(SdkUtils.getApp())) {
                int maxAge = 0; // 有网络时 设置缓存超时时间0个小时
                LogUtils.dLog("addNetWorkInterceptor"+ "网络已连接，缓存时间为：" + maxAge);
            ...
            LogUtils.eLog("cookeHeader1-----------"+cookeHeader);
            return response;
        };
    }
